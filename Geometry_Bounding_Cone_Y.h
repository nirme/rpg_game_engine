...
#ifndef _GEOMETRY_BOUNDING_CONE_Y
#define _GEOMETRY_BOUNDING_CONE_Y
...
class BoundingConeY : public BoundingCone
{
public:


	BoundingConeY( WorldPosition* _p_worldPosition, float _height, float _angle, D3DXVECTOR3* _apex = NULL );
	...
	inline virtual void update()
	{
		...
		if ( ( stamp = p_worldPosition->getWorldPositionTimestamp() ) != dw_worldPositionTimestamp )
		{
			...
			absDist = cashedRadius * ( abs( mx->_31 ) + abs( mx->_11 ) );
			...
			absDist = cashedRadius * ( abs( mx->_32 ) + abs( mx->_12 ) );
			...
			absDist = cashedRadius * ( abs( mx->_33 ) + abs( mx->_13 ) );
...
#endif //_GEOMETRY_BOUNDING_CONE_Y
