...
float4 ps_RenderLTDir(VS_FS_QUAD_IN_OUT Input) : COLOR0
{
	...
	// specular
	Output += lightInfoTable[2].xyz * mrt0.w * ( saturate(dot(mrt1.xyz, halfdir)), mrt1.w);
...
float4 ps_RenderLTOmni(VS_FS_QUAD_IN_OUT Input) : COLOR0
{
	...
	float3 Output = mrt0.xyz * lightInfoTable[1].xyz * dotNL;
	// specular
	Output += lightInfoTable[2].xyz * mrt0.w * pow(saturate(dot(mrt1.xyz, halfdir)), mrt1.w);
